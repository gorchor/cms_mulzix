...
<body>
                    ...
                    <div class="header-info">
                        ...
                        <div class="lang-right">
                            <form method="POST" action="{{ route('lang.change') }}" id="langForm">
                                ...
                                @php
                                    $current = $languages->where('id', session('session_lang_id'))->first();
                                @endphp
                                ...
                                <div class="custom-lang-dropdown" id="customLangDropdown">
                                    <div class="custom-lang-selected" id="customLangSelected">
                                        @if($current)
                                            <div class="lang-selected-left">
                                                <img
                                                    src="{{ asset('frontEnd/images/flags/' . strtolower($current->short_name) . '.png') }}"
                                                    alt="{{ $current->name }}"
                                                    class="lang-flag"
                                                >
                                                <span>{{ $current->short_name }}</span>
                                            </div>
                                        @else
                                            <div class="lang-selected-left">
                                                <span>LANG</span>
                                    ...
                                    <div class="custom-lang-menu" id="customLangMenu">
                                        @foreach($languages as $lang)
                                            <div class="custom-lang-item" data-id="{{ $lang->id }}">
                                                <img
                                                    src="{{ asset('frontEnd/images/flags/' . strtolower($lang->short_name) . '.png') }}"
                                                    alt="{{ $lang->name }}"
                                                    class="lang-flag"
                                                >
                                                <span>{{ $lang->name }}</span>
                                ...
                                <input type="hidden" name="lang_change_id" id="lang_change_id">
                            </form>
                        </div>
                        @endif
                ...
                <div class="col-lg-3 col-12">
                    <div class="logo flex">
                        <a href="{{ route('home.page') }}">
                            <img src="{{ asset('upload/'.$setting->logo) }}" alt="Logo">
                        </a>
                    </div>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var items = document.querySelectorAll('.custom-lang-item');
        ...
        if (dropdown && selected && menu) {
            ...
            items.forEach(function(item) {
                item.addEventListener('click', function (e) {
                    ...
                    document.getElementById('lang_change_id').value = id;
